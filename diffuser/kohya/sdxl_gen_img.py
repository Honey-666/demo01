...
class PipelineLike:
    ...
    def __call__(
        ...
        if crop_left is None:
            crop_left = 0
        emb1 = sdxl_train_util.get_timestep_embedding(torch.FloatTensor([original_height, original_width]).unsqueeze(0),
                                                      256)
        ...
        c_vector = torch.cat([emb1, emb2, emb3], dim=1).to(self.device, dtype=text_embeddings.dtype).repeat(batch_size,
                                                                                                            1)
        uc_vector = torch.cat([uc_emb1, emb2, emb3], dim=1).to(self.device, dtype=text_embeddings.dtype).repeat(
            batch_size, 1)
        ...
        if init_image is not None and self.clip_vision_model is not None:
            ...
            clip_vision_embeddings = self.clip_vision_model(pixel_values=pixel_values, output_hidden_states=True,
                                                            return_dict=True)
                ...
                if vae_batch_size >= batch_size:
                    ...
                    for i in tqdm(range(0, min(batch_size, len(init_image)), vae_batch_size)):
                        init_latent_dist = self.vae.encode(
                            (init_image[i: i + vae_batch_size] if vae_batch_size > 1 else init_image[i].unsqueeze(
                                0)).to(
                                self.vae.dtype
                            )
                        ).latent_dist
        ...
        if vae_batch_size >= batch_size:
            ...
            for i in tqdm(range(0, batch_size, vae_batch_size)):
                images.append(
                    self.vae.decode(
                        (latents[i: i + vae_batch_size] if vae_batch_size > 1 else latents[i].unsqueeze(0)).to(
                            self.vae.dtype)
                    ).sample
                )
...
def get_weighted_text_embeddings(
    ...
    if not skip_parsing:
        ...
        if uncond_prompt is not None:
            ...
            uncond_tokens, uncond_weights = get_prompts_with_weights(tokenizer, token_replacer, uncond_prompt,
                                                                     max_length - 2)
    else:
        ...
        if uncond_prompt is not None:
            ...
            uncond_tokens = [token[1:-1] for token in
                             tokenizer(uncond_prompt, max_length=max_length, truncation=True).input_ids]
...
def main(args):
        ...
        for i, network_module in enumerate(args.network_module):
            ...
            if args.network_weights and i < len(args.network_weights):
                ...
                network, weights_sd = imported_module.create_network_from_weights(
                    network_mul, network_weight, vae, [text_encoder1, text_encoder2], unet, for_inference=True,
                    **net_kwargs
                )
            else:
    ...
    if args.guide_image_path is not None:
        ...
        if len(guide_images) == 0:
            ...
            guide_images = None
    elif args.guide_image is not None:
        guide_images = []
        for img in args.guide_image:
            print(f"load image for ControlNet guidance: {img}")
    ...
    if args.seed is not None:
        ...
        predefined_seeds = [random.randint(0, 0x7FFFFFFF) for _ in
                            range(args.n_iter * len(prompt_list) * args.images_per_prompt)]
    ...
    for gen_iter in range(args.n_iter):
        ...
        def process_batch(batch: List[BatchData], highres_fix, highres_1st=False):
            ...
            for i, (
                    _, (_, prompt, negative_prompt, seed, init_image, mask_image, clip_prompt, guide_image),
                    _) in enumerate(
                batch):
        ...
        while args.interactive or prompt_index < len(prompt_list):
            ...
            for pi in range(args.images_per_prompt if len(raw_prompts) == 1 else len(raw_prompts)):
                ...
                if len(batch_data) == args.batch_size:
                    prev_image = process_batch(batch_data, highres_fix)
                    batch_data.clear()
...
def setup_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--original_height", type=int, default=None,
        help="original height for SDXL conditioning / SDXLの条件付けに用いるoriginal heightの値"
    )
    parser.add_argument(
        "--original_width", type=int, default=None,
    ...
    parser.add_argument(
        "--control_net_lllite_models", type=str, default=None, nargs="*",
