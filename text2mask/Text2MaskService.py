...
from TranslateUtil import zh_to_en
...
class Mask:
    def run(self, oss_url: str, mask_prompt: str, negative_mask_prompt: str, mask_precision: int, mask_padding: int,
            brush_mask_mode: int):
        ...
        try:
            ...
            # 变回PLI类型转换base64
            pli_img = gray_to_pil(median_blur_img)
            pli_img.save(output_buffer, format='JPEG')
            base64_str = base64.b64encode(output_buffer.getvalue()).decode("utf-8")
            return_json['status'] = 200
            return_json['img_base64'] = base64_str
            my_logger.info("handle success..........")
        except Exception as e:
            my_logger.exception('handle exception:')
            return_json["error"] = str(e)
            return_json["img_base64"] = ''
...
class Item(BaseModel):
    ...
    prompt: str
    negative_prompt: str
    mask_padding: Optional[int] = 0
    mask_precision: Optional[int] = 100
    smooth: Optional[int]


@app.post("/mask")
async def extract(item: Item):
    if item.img_url is None:
        return {"status": 404, "img_base64": ''}
    prompt = item.prompt
    negative_prompt = item.negative_prompt
    print('img_url=', item.img_url, ' ,prompt=', prompt, ' ,negative_prompt=', negative_prompt,
          ' ,mask_padding=', item.mask_padding, ' ,mask_precision=', item.mask_precision)

    en_prompt = zh_to_en(prompt) if prompt else prompt
    en_negative_prompt = zh_to_en(negative_prompt) if negative_prompt else negative_prompt
    print('after translate en_prompt=', en_prompt, ' ,en_negative_prompt=', en_negative_prompt)
    return Mask().run(oss_url=item.img_url, mask_prompt=en_prompt, negative_mask_prompt=en_negative_prompt,
                      mask_precision=item.mask_precision,
                      mask_padding=item.mask_padding, brush_mask_mode=0)


if __name__ == "__main__":
    print('this is main, start........', str(datetime.datetime.now()))
    uvicorn.run(app='Text2MaskService:app', host="0.0.0.0", port=9901, reload=False)
